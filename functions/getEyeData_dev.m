function [eye] = getEyeData_dev(info, timePoints, doPlot)
...
et_processed = [info.expDate,'_', num2str(info.exp),'_',info.subject,'_eye_processed.mat'];
et_raw = [info.expDate,'_', num2str(info.exp),'_',info.subject,'_eye.mat'];
et_movie = [info.expDate,'_', num2str(info.exp),'_',info.subject,'_eye.mj2'];

    if exist(fullfile(etRepo_old, info.subject, info.expDate, num2str(info.exp)))  
    
        etFolder = fullfile(etRepo_old, info.subject,info.expDate, num2str(info.exp));
    else
...
eye.area = eye.pupil; %are in px
eye.pupil = eye.pupil/prctile(eye.pupil,10); % consider z-scoring  >>zscore()
eye.x = gaussFilt(eye.x', 3)-nanmean(eye.x); %% -mean(eye.x)
eye.y = gaussFilt(eye.y', 3)-nanmean(eye.y); %% -mean(eye.x)
